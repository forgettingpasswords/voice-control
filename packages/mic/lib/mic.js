...
var mic = function mic(options) {
    ...
    that.start = function start() {
	      ...
	      // Use default pipewire recording device instead.
	      // TODO Make configurable to maybe pipe in other audio streams
	      format = formatEncoding.toLowerCase() + bitwidth;
	      const args = ['-r', '--channels', channels, '--rate', rate, '--format', format, '-'];
	      audioProcess = spawn('pw-cat', args, audioProcessOptions);
            }
